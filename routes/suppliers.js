...
router.post('/', auth, async (req, res) => {
  ...
  var { data, error } = await supabase
    .from('suppliers').insert({ name, phone, email, address, notes }).select().single();
  if (error) return res.status(500).json({ error: 'Error interno' });
...
// GET /api/suppliers/purchases — todas las compras con items
...
// POST /api/suppliers/purchases — registrar compra y actualizar stock
router.post('/purchases', auth, async (req, res) => {
  ...
  var { data: purchase, error: pErr } = await supabase
    .from('purchases')
    .insert({ supplier_id: supplierId || null, supplier_name: supplierName, total, notes: notes || null, registered_by: req.user.name })
    .select().single();
  ...
  // Actualizar stock y costo de cada producto en inventario
  for (var i = 0; i < items.length; i++) {
    var it = items[i];
    if (!it.productId) continue;
    var { data: prod } = await supabase.from('products').select('stock,cost').eq('id', it.productId).single();
    if (!prod) continue;
    ...
    await supabase.from('products').update(updateFields).eq('id', it.productId);
  }
  ...
  // Devolver compra completa con items
  var { data: full } = await supabase
    .from('purchases').select('*, purchase_items(*)').eq('id', purchase.id).single();
